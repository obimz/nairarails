...
- `webhook.ts` — the Nomba webhook envelope shape from contract Section 4.4/4.5
...
- `verifySignature.ts` — `verifyNombaWebhook(rawBody: Buffer, signatureHeader: string, secret: string): boolean`, implementing the confirmed scheme from contract 4.6: `HMAC-SHA256(secret, rawBody)`, hex digest, compared with a timing-safe equality check (`crypto.timingSafeEqual`, not `===`)
...
- `tests/verifySignature.test.ts` — one known-good signature (accept), one tampered payload or wrong secret (reject)
...
4. **`POST /api/v1/webhooks/nomba`** — the most carefully built route in the system. Mounted with `express.raw({ type: "application/json" })`, not the global JSON parser, because signature verification (Phase 2) needs the untouched raw bytes. Flow: verify signature → check `webhook_events` for existing `requestId` → insert the event row → look up the order by reference → call `classify()` (Phase 2) → update order status → call `executeSplits()` (which itself calls `lookupBankAccount` then `transferToBank` from Phase 4, per party) → always respond `200` once the event is safely recorded, regardless of outcome.
